...
$sql = "SELECT * FROM stock WHERE arch='0' order by name ASC";
...
if(isset($_POST['add_submit'])){
	...
	$name = $_POST['stock'];
	$price = $_POST['price'];
	$sql = "SELECT * FROM stock WHERE name = '$name' and arch='0'";
	$result = $db ->query($sql);
	...
	$db->query("INSERT INTO stock (name, price) VALUES ('$name', '$price')");
...
if(isset($_GET['delete'])){
	$did = $_GET['delete'];
	$db->query("UPDATE stock SET arch='1' WHERE id = '$did'");
...
<table class="table table-bordered stable-striped table-auto table-condensed">
	...
	<?php while($stock = mysqli_fetch_assoc($results)) : ?>
		<tr>
			<td><button type="button" class="btn btn-lg btn-default" onCLick="detailsmodal(<?= $stock['id']; ?>)">Update Stock</button></td>
