...
$sql = "SELECT * FROM staff WHERE type='waiter' and id='$id'";
$result = $db ->query($sql);
$waiter = mysqli_fetch_assoc($result);
?>
				...
				<div class="container-fluid">
					...
					<span id="modal_errors" class="bg-danger"></span>
						<div class="col-sm-12">
							<h4><?= $waiter['name']; ?></h4>
							<hr>
							<form action="waiter/login.php" method="post">
							<!-- All Inputs Get sent to footer jQuery and Ajax script, which send to add_cart -->
							<input type="hidden" name="name" id="name" value="<?=$waiter['name'] ;?>"/>
							<input type="hidden" name="id" id="id" value="<?=$id ;?>"/>
								<div class="form-group">
									<label for="password"> Password:</label>
									<input type="password" name="password" id="password" class="form-control" required />
								</div>
								<button class="btn btn-warning" type="submit" name="log"> Login</button>
