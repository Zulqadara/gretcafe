...
// document properties
$pdf->SetAuthor('...');
$pdf->SetTitle('ACTIVITIES PER WAITER REPORT');
...
$pdf->Cell(40,10,'Activities Per Waiter - '.$date);
...
$pdf->setXY(10, 40); 
$pdf->Cell(40, 10, "Waiter", 1, 0, "L", 1);   // CHANGE THESE TO REPRESENT YOUR FIELDS
$pdf->Cell(80, 10, "Sales Made", 1, 0, "L", 1);
...
$query_result = "SELECT staff.name as name, COUNT(waiterid) as w, `order`.id, waiterid, DAY(datetime) as day, MONTH(datetime) as month , YEAR(datetime) as year 
...
{
        $pdf->Cell(40, 8, $row['waiterid'].' - '.$row['name'], 1);   // CHANGE THESE TO REPRESENT YOUR FIELDS
        $pdf->Cell(80, 8, $row['w'], 1);
        $y += 8;
